...
function ProfileCreate () {
    ...
    if (isError && error) {
        ...
        if ('status' in error) {
            const errMsg = 'error' in error ? error.error : 'have to give up because dunno what error.data is'; // TODO: uhh huh?
            return <h1>Error creating profile: {error.status} {errMsg}</h1>;
        } else {
            const errMsg = error.message;
            return <h1>Error creating profile: {errMsg}</h1>;
        }
    } else { // isUninitialized || isLoading
        return (
